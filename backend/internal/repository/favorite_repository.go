...
func (r *FavoriteRepository) Delete(id uint) error { return translate(r.db.Delete(&model.Favorite{}, id).Error) }
...
func (r *FavoriteRepository) ListByUser(userID uint) ([]model.Favorite, error) {
	...
	if err := translate(r.db.Where("user_id = ?", userID).Order("id DESC").Find(&items).Error); err != nil {
		return nil, err
	}
	return items, nil
}
